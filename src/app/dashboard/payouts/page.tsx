...
export default function AllPayoutsPage() {
  ...
  const [filtered, setFiltered] = useState<QBPayout[]>([]);
  ...
  const fetchPayouts = useCallback(async () => {
    ...
    try {
      ...
      if (!res.ok) throw new Error(String(body?.error?.message || body?.message || "Failed to fetch payouts."));
      startTransition(() => {
        setConnected(body.connected ?? false);
        setPayouts(body.payouts || []);
        setFiltered(body.payouts || []);
      });
    } catch (err: unknown) {
      startTransition(() => setError(err instanceof Error ? err.message : "Failed to load payouts."));
    } finally {
  ...
  const totalAmount = filtered.reduce((acc, p) => {
    const num = Number(p.amount.replace(/[^0-9.-]+/g, ""));
    ...
    <div className="min-h-screen bg-[#060606] text-white">
      <div className="mx-auto w-full max-w-[1100px] px-4 py-8 sm:px-6">
        ...
        <div className="flex flex-col gap-1 mb-8">
          <h1 className="text-2xl font-bold tracking-tight text-white">Outgoing Payouts</h1>
          <p className="text-sm text-neutral-500">Full history of payouts sent to talent, vendors, and agencies.</p>
        </div>
        ...
        <div className="rounded-xl border border-[#222] bg-[#0D0D0D] overflow-hidden">
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center py-24 text-center px-6">
              ...
              <button onClick={fetchPayouts} className="text-xs font-bold text-white underline">Try again</button>
            </div>
          ) : !connected ? (
            <div className="flex flex-col items-center justify-center py-24 text-center px-6">
          ...
          {!loading && !error && filtered.length > 0 && (
            <div className="flex items-center justify-between border-t border-[#222] px-5 py-4">
              ...
              <div className="flex items-center gap-3">
                <span className="text-xs text-neutral-500">Total Paid Out</span>
